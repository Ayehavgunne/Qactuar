...
class AccessLogger(QactuarLogger):
    def __init__(self, name: str, level: int):
        super().__init__(name, level)
...
def create_access_logger() -> AccessLogger:
    logger = AccessLogger("qt_http_access", INFO)
...
def create_child_logger() -> QactuarLogger:
    logger = QactuarLogger("qt_child", INFO)
    handler = StreamHandler()
